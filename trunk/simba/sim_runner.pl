...
my $testName = "iil_1";
my $testDir = "/tmp/simba/iil_1_new_traces_reservation";
my $multiplier = "1";
...
foreach (@tests) 
{
	...
	my $cmd = "echo algo $algo pid \$\$ dir $specificTestDir ; cd $specificTestDir; /usr/intel/pkgs/java/1.6.0.31-64/bin/java -Xmx15g -Dhosts-file=$hostsFile -Dhost-memory-multiplier=$multiplier -Djobs-file=$jobsFile -Dgrader=$algo -jar $simulatorJar > sim.out &";
#	print "executing $cmd\n"; 
  	system("$cmd");
}
