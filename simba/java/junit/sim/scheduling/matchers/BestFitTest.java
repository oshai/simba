...
public class BestFitTest
{
	...
	public void testNoAccpetingHostNotFit()
	{
		...
		assertNull(new BestFit().match(Job.create((long) 1).cores(1).build(), hosts));
	}
	...
	public void testAccpetingHostFit()
	{
		...
		assertEquals(host, new BestFit().match(Job.create((long) 1).cores(1).build(), hosts));
	}
	...
	public void testAccpetingBestHost()
	{
		...
		assertEquals(host3, new BestFit().match(Job.create((long) 1).cores(1).memory(4).build(), hosts));
	}
