...
public class MixFitTest
{
	...
	public void testNoAccpetingHostNotFit()
	{
		...
		assertNull(new MixFit().match(Job.Builder.create(1).cores(1).build(), hosts));
	}
	...
	public void testAccpetingHostFit()
	{
		...
		assertEquals(host, new MixFit().match(Job.Builder.create(1).cores(1).build(), hosts));
	}
	...
	public void testAccpetingBestHost()
	{
		...
		assertEquals(host3, new MixFit().match(Job.Builder.create(1).cores(1).memory(4).build(), hosts));
	}
	...
	{
		Host host1 = Host.Builder.create().cores(2).memory(16).build();
		Host host2 = Host.Builder.create().cores(1).memory(8).build();
		List<Host> hosts = newArrayList(host1, host2);
		assertEquals(host2, new MixFit().match(Job.Builder.create(1).cores(1).memory(4).build(), hosts));
	}
